...
{
    Admin_behavioral_full_faculty x = new Admin_behavioral_full_faculty();
        ...
        if (uploadfulltimefaculty_image.HasFile)
        {
            if (uploadfulltimefaculty_image.PostedFile.ContentType == "image/jpeg")
            {
                if (uploadfulltimefaculty_image.PostedFile.ContentLength < 600000)
                {
                    ...
                    uploadfulltimefaculty_image.SaveAs(Server.MapPath("~/Behavioral Fulltime Faculty Image/" + fname));
                    string qry = "insert into Admin_Behavioral_Full_Faculty values('" + uploadfulltimefaculty_image.FileName + "','" + txtfullfacu_name.Text + "','" + txtfullfacu_quali.Text + "','" + txtfullfacu_desig.Text + "','" + txtfullfacu_email.Text + "','" + txtfullfacu_expr.Text + "','" + txtfullfacu_speci.Text + "','" + txtfullfacu_publi.Text + "')";
                    x.admin_behavioral_full_faculty_insert(qry);
                    lbl_path.Text = "file upload successfully..";
                    Response.Redirect("Admin_Behavioral_Full_Faculty.aspx");
                }
                else
                {
